...
-->
网络是Kubernetes的一个核心部分，但是去准确的了解它的运行的确是充满挑战的。这里有四个显著的网络问题需要处理：
1. 容器和容器之间的高解耦通讯：这个是通过[pods](/docs/concepts/workloads/pods/pod/) 和 本地主机得以解决。
2. Pod和Pod之间的通讯，这个是本片文档最原始的关注点。
3. Pod和Service之间的通讯：这个问题可以参考[services](/docs/concepts/services-networking/service/)。
4. 外部和Service之间的通讯： 这个问题可以参考[services](/docs/concepts/services-networking/service/)。
...
Kubernetes是关于在应用程序之间共享机器的。通常，共享机器需要确保两个应用程序不尝试使用相同的端口。
跨多个开发人员的协调端口在规模上是非常困难的，并且使用户面临超出他们控制范围的集群级别的问题。


动态端口分配给系统带来了很多复杂性-每一个应用程序必须以端口为标志，API服务器必须知道如何
在配置块中插入动态端口号，服务必须知道如何找到对方等。Kubernetes不是处理这件事，而是采取了
不同的方法。

## Kubernetes的网络模型

每个`Pod`都有自己的IP地址。这意味着您不需要显式地在`Pods`之间创建链接，而且您几乎不需要处理
将容器端口映射到宿主港口的问题。这创建了一个干净的、向后兼容的模型，
其中可以从端口分配、命名、服务发现、负载平衡、应用程序配置和迁移的角度来处理`Pods`，
就像VM或物理主机一样。

Kubernetes对任何网络实现提出了以下基本要求(除非是有意的网络分段策略)：
    * 节点上的pod可以与所有节点上的所有pod通信，没有NAT
    * 节点上的代理(例如系统守护进程、kubelet)可以与该节点上的所有节点通信。
注意：对于那些支持在主机网络中运行的`Pods`的平台(例如Linux)：
    * 节点主机网络中的pod可以与所有没有NAT的节点上的所有pod通信。
